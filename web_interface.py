...
def display_dataset_viewer():
    ...
    if selected_example is not None:
        ...
        st.markdown(f"<div class='document-card'>{example['question']}</div>", unsafe_allow_html=True)
        
        st.markdown("#### Answer")
        st.markdown(f"<div class='document-card'>{example['answer']}</div>", unsafe_allow_html=True)
...
def display_pipeline_config():
    ...
    if submit_button:
        ...
        if use_existing == "Use Existing Questions":
            ...
            # Generate new questions using the variables from the form
        ...
        if use_existing == "Use Existing Questions":
            command = f"python teacherforge.py run-pipeline --questions {questions_path} --output {output_dir} --top-k {top_k} --min-confidence {min_confidence} --format {dataset_format}"
        else:
            command = f"python teacherforge.py run-pipeline --questions {questions_path} --output {output_dir} --num {num_questions} --domain \"{domain}\" --top-k {top_k} --min-confidence {min_confidence} --format {dataset_format}"
